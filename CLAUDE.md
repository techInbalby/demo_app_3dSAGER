...
- The legacy confidence cutoff `CONFIDENCE_THRESHOLD = 0.5` is hardcoded at `app.py:37` and `tasks._LEGACY_THRESHOLD`; the alignment match threshold is **0.40** (`pipeline_stages.DEFAULT_MATCH_THRESHOLD`) — not the source-paper's 0.65. We tuned downward because the demo uses a 500 m DisasterSim translation (vs. the source paper's ±100 km, which would put cands outside the WGS84 projection's valid extent and break viewer sub-stage 4a). In the 500 m regime the look-alikes the classifier surfaces stay spatially closer to real index buildings, compressing the score distribution; the actual demo-regime peak F1 sits at 0.40 (F1 ≈ 0.86, P ≈ 0.77, R ≈ 0.96 on the locked Hague inputs). Don't "fix" this by raising it back — that's how you get F1 ≈ 0.77 advertised on a pipeline that actually works fine.
